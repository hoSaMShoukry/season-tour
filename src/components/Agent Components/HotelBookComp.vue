<template>
    <div class="hotel-details bg-white p-4 rounded-1">
        <div class="container min-vh-100">
            ...
            <div class="inner py-5 d-flex gap-4 align-items-center flex-column">
                <div class="img-cont w-50">
                    <img class="img-fluid" :src="`https://api.seasonsge.com/images/${hotelDetails.main_image}`" alt="" />
                </div>
                <div class="hotel-info flex-fill w-100">
                    ...
                    <!-- <div class="group-2 mt-3 d-flex gap-3">
                        <label class="w-100">
                            <span class="d-block mb-1">{{$t('hotelBook.details.room.head')}}:</span>
                            <select class="rounded-1 p-2 px-3 w-100" name="room type" v-model="hotelBooking.room_type">
                                <option value="single room">{{$t('hotelBook.details.room.single')}}</option>
                                <option value="double room">{{$t('hotelBook.details.room.double')}}</option>
                                <option value="triple room">{{$t('hotelBook.details.room.triple')}}</option>
                            </select>
                        </label>
                        <label class="w-100">
                            <span class="d-block mb-1">{{$t('hotelBook.details.childReservation.head')}}:</span>
                            <select class="rounded-1 p-2 px-3 w-100" name="child reservation"
                                v-model="hotelBooking.child_room_type">
                                <option value="none">{{$t('hotelBook.details.childReservation.none')}}</option>
                                <option value="child without bed">{{$t('hotelBook.details.childReservation.without')}}</option>
                                <option value="child with bed">{{$t('hotelBook.details.childReservation.with')}}</option>
                            </select>
                        </label>
                    </div>
                    <div class="group-3 mt-3 d-flex gap-3 flex-md-row flex-column">
                        <label class="w-100">
                            <span class="d-block mb-1">{{$t('hotelBook.details.roomPrice')}}:</span>
                            <span class="d-block p-2 px-3 rounded-1 bg-secondary bg-opacity-25">
                                {{ USDollar.format(roomAndChildPrices.room_price || 0) }}
                            </span>
                        </label>
                        <label class="w-100">
                            <span class="d-block mb-1">{{$t('hotelBook.details.childPrice')}}:</span>
                            <span class="d-block p-2 px-3 rounded-1 bg-secondary bg-opacity-25">
                                {{ USDollar.format(roomAndChildPrices.child_price || 0) }}
                            </span>
                        </label>
                    </div> -->
                    ...
                    <!-- <div class="group-6 mt-3 d-flex gap-3 flex-md-row flex-column">
                        <label class="w-100">
                            ...
                            <input :class="`rounded-1 w-100 p-2 px-3 ${validation.start_date.$error ? 'error' : ''}`"
                                type="date" v-model="hotelBooking.start_date">
                        </label>
                        <label class="w-100">
                            <span class="d-block mb-1">{{ $t('hotelBook.details.tDate') }}:</span>
                            <input :class="`rounded-1 w-100 p-2 px-3 ${validation.start_date.$error ? 'error' : ''}`"
                                type="date" v-model="hotelBooking.end_date">
                        </label>
                    </div>
                    ...
                    <label class="w-100 mt-3">
                        ...
                        <span class="d-block p-2 px-3 rounded-1 bg-secondary bg-opacity-25">
                            {{ USDollar.format(hotelBooking.agent_discount || 0) }}
                        </span>
                    </label>
                    <label class="w-100 mt-3">
...
import HotelBook from "../HotelBook.vue";
...
const hotelBooking = ref({
    ...
    total: 0,
    agent_discount: computed(() => (hotelBooking.value.total * userInfo.value.discount) / 100),
    tax: computed(() => parseFloat(hotelDetails.value.tax).toFixed(2)),
    net_total: computed(() => {
        return (+hotelBooking.value.total + ((hotelBooking.value.tax / 100) * +hotelBooking.value.total)) - hotelBooking.value.agent_discount
    }),
    account_name: "",
});
