<template>
    <div class="flight-tabel p-4 rounded-1 bg-white">
        ...
        <div class="responsive-table overflow-auto mt-5" dir="ltr">
            ...
            <table class="table">
                <thead class="">
                    <div class="d-flex justify-content-between border-bottom pb-2" style="padding-left: 9px; padding-right: 9px;">
                        
                        <div style="font-weight: 700;">Flight Information	</div>
                        <div style="font-weight: 700;">Non-Refundable</div>
                    </div>
                </thead>
            </table>
            <div class="d-flex main-cont" v-if="searchResults.length > 0 || suggestions.length > 0"
                v-for="(item, index) in  searchResults.length > 0 ? searchResults : suggestions" :key="index">
                <div class="detials d-flex flex-column flex-fill">
                    <div class="go d-flex flex-fill">
                        ...
                        <div class="text-center">
                            <span v-if="item.numStops == '0'" class="d-block">Direct</span>
                            <span v-else class="d-block">{{ item.numStops }} Stop <br> {{ item.locStops }}</span>

                            <span class="d-block" v-if="item.numStops > '0'">Stopping places: Dubai</span>
                            <span class="d-block" v-if="item.numStops > '0'">Layover:Dubai 5h 50m</span>
                            <span class="duration d-block mt-2 pt-2 text-muted position-relative">
                                <!--عدد ساعات الذهاب-->
                                Trip Duration: {{ item.hours_arriv ? item.hours_arriv : '2.5' }} hours
                            </span>
